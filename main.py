...
st.markdown("""
    ...
    * {
    ...
    /* إجبار القوائم والجداول على اليمين */
...
# 3. جلب البيانات بتحديث إجباري لمنع الكاش
@st.cache_data(ttl=0)
                ...
                with st.form(key="attendance_form_secure", clear_on_submit=True):
                    ...
                    if st.form_submit_button("✅ اعتماد كشف الحضور", use_container_width=True):
                        if selected:
                            ...
                            time.sleep(2) # تأخير للسماح لجوجل شيت بالحفظ
                            st.rerun()
                        else:
            ...
            with st.form(key="add_student_form_secure", clear_on_submit=True):
                ...
                if st.form_submit_button("إضافة الطالب الآن", use_container_width=True):
                    if name_in:
                        ...
                        st.cache_data.clear() # مسح الذاكرة
                        st.success(f"تمت الإضافة بنجاح!")
                        time.sleep(2) # الانتظار لثانيتين حتى يسجل جوجل شيت الاسم
                        st.rerun() # تحديث الصفحة ليظهر في قائمة التحضير فوراً
                    else: 
        ...
        with sub3:
            ...
            if st.button("🔍 تجهيز التقرير التفصيلي", use_container_width=True):
                if not l_list.empty:
                    ...
                    # حساب أيام النشاط الفعلية في هذه الفترة
                    ...
                    for _, student in m_list.iterrows():
                        ...
                        # إضافة كافة التفاصيل لتقرير شامل
                        rep_data.append({
                            "الاسم": student['الاسم'],
