...
with tab_stats:
    if not m_list.empty:
        ...
        display_df["أيام الحضور"] = display_df["الاسم"].apply(lambda n: l_list[l_list["الاسم"] == n]["التاريخ"].nunique() if not l_list.empty else 0)
        display_df["النسبة المئوية"] = ((display_df["أيام الحضور"] / total_activity_days * 100).round(1).astype(str) + "%") if total_activity_days > 0 else "0%"
        st.table(display_df)
        ...
        with sub3:
            ...
            if st.button("🔍 تجهيز التقرير التفصيلي", use_container_width=True):
                if not l_list.empty:
                    ...
                    res_df = pd.DataFrame(rep_data).sort_values(by="الاسم", ignore_index=True)
