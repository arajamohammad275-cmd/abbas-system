...
with tab_stats:
    if m_list.empty:
        ...
        m_list['أيام الحضور'] = m_list['الاسم'].apply(
            lambda n: l_list[l_list['الاسم']==n]['التاريخ'].dt.date.nunique() if not l_list.empty else 0
        )
        m_list['النسبة المئوية'] = (
            (m_list['أيام الحضور']/total_activity_days*100).round(1).astype(str)+'%' if total_activity_days>0 else '0%'
        )
